...
/* Between us_socket_adopt_tls and us_socket_start_tls_handshake: no TLS output
 * (the previous owner's queued plaintext reaches the wire first) and inbound
 * bytes are dispatched undecrypted. */
void us_socket_hold_tls_handshake(us_socket_r s) nonnull_fn_decl;
