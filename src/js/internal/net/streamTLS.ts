...
// handle-backed net.Socket is read and written natively (like under node's
// TLSWrap), hands over what it had already buffered, and has our output held
// while it still has writes queued (`deferHandshake`); any other Duplex is
// driven from its events (node's JSStreamSocket). Same split as
...
function upgradeStreamToTLS(owner: { destroyed: boolean }, connection, options) {
  ...
  // A Duplex orders our writes behind its own by itself.
  if (!transport) options.deferHandshake = false;
